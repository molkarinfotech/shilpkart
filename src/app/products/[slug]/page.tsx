    ...
    <main className="min-h-screen bg-sand-50">
      ...
      <section className="mx-auto grid max-w-7xl gap-12 px-5 py-12 lg:grid-cols-2 lg:py-20">
        ...
        <div className="flex flex-col justify-center">
          <p className="text-[11px] font-semibold uppercase tracking-[.22em] text-clay-600">{product.artisan_location}</p>
          <h1 className="mt-4 font-display text-4xl tracking-tight text-ink-900 md:text-5xl">{product.title}</h1>
          <p className="mt-4 text-lg text-ink-500">By {product.artisan_name}</p>
          <p className="mt-8 font-display text-3xl text-ink-900">
            {formatINR(product.price_inr)}
            {product.compare_at_price_inr ? (
              <span className="ml-2 text-lg font-sans font-normal text-ink-400 line-through">{formatINR(product.compare_at_price_inr)}</span>
            ) : null}
          </p>
          <p className="mt-7 text-lg leading-8 text-ink-600">{product.description}</p>
          <div className="mt-8">
            <AddToBag product={product} />
          </div>
          <div className="mt-10 border-t border-ink-200 pt-7">
            <p className="text-[11px] font-semibold uppercase tracking-[.22em] text-clay-600">The maker’s story</p>
            <p className="mt-3 leading-8 text-ink-600">{product.story}</p>
          </div>
        </div>
      </section>
    </main>

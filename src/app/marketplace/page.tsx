    ...
    <main className="min-h-screen bg-sand-50">
      ...
      <section className="mx-auto max-w-7xl px-5 py-12">
        <div className="mb-8 flex flex-wrap gap-2">
          ...
          {categories.map((item) => (
            <a key={item} href={`/marketplace?category=${item}`} className="rounded-full border border-ink-200 bg-sand-50 px-4 py-2 text-sm font-semibold capitalize text-ink-700 transition hover:border-clay-400 hover:text-clay-700">{item}</a>
          ))}
        </div>
        ...
        {visible.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{visible.map((product) => <ProductCard key={product.id} product={product} />)}</div> : <div className="mt-10 rounded-2xl border border-dashed border-ink-300 bg-sand-100 p-12 text-center"><p className="font-display text-xl text-ink-800">This craft collection is coming soon.</p><a href="/marketplace" className="mt-4 inline-block font-semibold text-clay-700">View all pieces →</a></div>}
      </section>
    </main>
  );
}
